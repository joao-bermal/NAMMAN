...
export class T3KClient {
   ...
   *
   * `path` may be an API path (e.g. `/api/v1/user`) or an absolute URL such as
   * a `model_url` returned by the API — absolute URLs are used as-is so model
   * files can be fetched with the Bearer token (and auto-refresh) intact.
   */
  ...
  /** Favorite a tone on behalf of the authenticated user. Idempotent. */
  async favoriteTone(id: number | string): Promise<void> {
    const res = await this.fetch(`/api/v1/tones/${id}/favorite`, { method: 'PUT' });
    if (!res.ok) throw new Error(`favoriteTone failed: ${res.status}`);
  }

  /** Remove a tone from the authenticated user's favorites. Idempotent (204). */
  async unfavoriteTone(id: number | string): Promise<void> {
    const res = await this.fetch(`/api/v1/tones/${id}/favorite`, { method: 'DELETE' });
    if (!res.ok && res.status !== 204) throw new Error(`unfavoriteTone failed: ${res.status}`);
  }

...
export interface Tone {
  ...
  tags: Tag[];
  models_count: number;
  // Per-architecture breakdown (NAM tones) — present on GET /tones/{id} and search.
...
export interface SearchTonesParams {
  ...
  gears?: Gear[];
  sizes?: Size[];
  // 1 (A1), 2 (A2), or 'custom'. Omit for legacy A1 + Custom.
  architecture?: number | ArchitectureVersion;
}
